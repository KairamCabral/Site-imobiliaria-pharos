/**
 * Types para Sistema de Tracking Avançado - SSGTM
 * Imobiliária Pharos
 */

export interface UserData {
  email?: string;
  phone?: string;
  firstName?: string;
  lastName?: string;
  city?: string;
  state?: string;
  country?: string;
  zipCode?: string;
}

export interface PropertyData {
  id: string;
  code: string;
  title: string;
  price: number;
  bedrooms?: number;
  bathrooms?: number;
  suites?: number;
  parkingSpaces?: number;
  area?: number;
  type: string;
  city: string;
  state: string;
  neighborhood?: string;
  realtor?: {
    id: string;
    name: string;
  };
}

export interface TrackingParams {
  event: string;
  eventCategory?: string;
  eventLabel?: string;
  value?: number;
  currency?: string;
  userData?: UserData;
  propertyData?: PropertyData;
  customParams?: Record<string, any>;
}

export interface EnhancedUserData {
  em?: string; // hashed email
  ph?: string; // hashed phone
  fn?: string; // hashed first name
  ln?: string; // hashed last name
  ct?: string; // hashed city
  st?: string; // hashed state
  country?: string; // hashed country
  zp?: string; // hashed zip code
}

export interface ClientHints {
  platform?: string;
  platformVersion?: string;
  model?: string;
  browserVersion?: string;
  mobile?: boolean;
  brands?: Array<{ brand: string; version: string }>;
}

export interface MarketingParams {
  utm_source?: string;
  utm_medium?: string;
  utm_campaign?: string;
  utm_term?: string;
  utm_content?: string;
  referrer?: string;
  gclid?: string;
  fbclid?: string;
}

export interface TrackingPayload {
  event: string;
  event_category?: string;
  event_label?: string;
  value?: number;
  currency?: string;
  
  // Session & User
  session_id: string;
  user_id?: string;
  client_id?: string;
  
  // Facebook Pixel
  fbp?: string;
  fbc?: string;
  
  // Google Ads
  gclid?: string;
  
  // Enhanced Conversions (hashed user data)
  user_data?: EnhancedUserData;
  
  // Property data (ecommerce)
  ecommerce?: {
    items: Array<{
      item_id: string;
      item_name: string;
      item_brand: string;
      item_category: string;
      item_category2?: string;
      item_category3?: string;
      price: number;
      quantity: number;
      item_variant?: string;
    }>;
    value: number;
    currency: string;
  };
  
  // Client Hints
  client_hints?: ClientHints;
  
  // Marketing params
  utm_source?: string;
  utm_medium?: string;
  utm_campaign?: string;
  utm_term?: string;
  utm_content?: string;
  referrer?: string;
  
  // Custom params
  [key: string]: any;
  
  // Metadata
  page_url: string;
  page_title: string;
  screen_resolution: string;
  viewport_size: string;
  timestamp: string;
}

export interface MetaConversionAPIPayload {
  data: Array<{
    event_name: string;
    event_time: number;
    event_id?: string;
    action_source: string;
    event_source_url: string;
    user_data: {
      em?: string;
      ph?: string;
      fn?: string;
      ln?: string;
      ct?: string;
      st?: string;
      zp?: string;
      country?: string;
      client_ip_address?: string;
      client_user_agent?: string;
      fbp?: string;
      fbc?: string;
    };
    custom_data?: {
      currency: string;
      value: number;
      content_name?: string;
      content_category?: string;
      content_ids?: string[];
      content_type?: string;
      num_items?: number;
    };
  }>;
}

export interface GoogleAdsConversionPayload {
  conversion_action: string;
  conversion_time: string;
  conversion_value: number;
  currency_code: string;
  gclid?: string;
  user_identifiers?: Array<{
    hashed_email?: string;
    hashed_phone_number?: string;
    address_info?: {
      hashed_first_name?: string;
      hashed_last_name?: string;
      city?: string;
      state?: string;
      country_code?: string;
      postal_code?: string;
    };
  }>;
}

// Eventos do funil imobiliário
export enum TrackingEvents {
  // Page views
  PAGE_VIEW = 'page_view',
  
  // Interesse
  PROPERTY_VIEW = 'view_item',
  PROPERTY_GALLERY_VIEW = 'property_gallery_view',
  PROPERTY_FAVORITE = 'add_to_wishlist',
  PROPERTY_SHARE = 'share',
  PROPERTY_COMPARE = 'property_compare',
  
  // Consideração
  PROPERTY_CONTACT_VIEW = 'begin_checkout',
  PROPERTY_CONTACT_START = 'add_payment_info',
  FILTER_APPLY = 'filter_change',
  SEARCH = 'search',
  
  // Conversão
  LEAD = 'generate_lead',
  SCHEDULE_VISIT = 'purchase',
  CONTACT_WHATSAPP = 'contact_whatsapp',
  CONTACT_PHONE = 'contact_phone',
  
  // Engajamento
  VIDEO_VIEW = 'video_view',
  MAP_INTERACTION = 'map_interaction',
  VIRTUAL_TOUR = 'virtual_tour',
}

export enum EventCategory {
  PROPERTY_ENGAGEMENT = 'Property Engagement',
  LEAD_GENERATION = 'Lead Generation',
  CONVERSION = 'Conversion',
  REMARKETING = 'Remarketing',
  ENGAGEMENT = 'Engagement',
}

